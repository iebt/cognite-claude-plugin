...
- At the .NET upgrade step, add a verification step to confirm if the .NET version was upgraded successfully and if the current terminal has the correct version. Check if user is using asdf, if so, list current installed versions (asdf list dotnet) and set the correct version
